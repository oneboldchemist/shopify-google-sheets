...
• Detaljerad utskrift: varje avdrag från Butik 1 loggas rad‑för‑rad + summering
"""
...
def log(msg: str, *, lvl: str = "INFO"):
    ...
    print(f"\x1b[{COLORS.get(lvl,'37')}m[{ts}] {msg}\x1b[0m")
...
def to_int(x) -> int:
    ...
    log(f"⚠️ ogiltigt tal: {x!r}", lvl="WARN")
    return 0
...
def inventory(shop: str, tok: str, loc: int, vmap: Dict[str, Variant]) -> Dict[str, int]:
    ...
    for i in range(0, len(ids), 50):
        levels = shopify(
            ...
            params={"inventory_item_ids": ",".join(ids[i : i + 50]), "location_ids": loc},
...
def ensure_trackable(shop: str, tok: str, vid: int):
    ...
    if DRY_RUN:
        return
    shopify(shop, tok, "PUT", f"/variants/{vid}.json", json={"variant": {"id": vid, "inventory_management": "shopify"}})

def connect(shop: str, tok: str, iid: int, loc: int):
    try:
        if DRY_RUN:
            return
        shopify(shop, tok, "POST", "/inventory_levels/connect.json", json={"location_id": loc, "inventory_item_id": iid})
...
def adjust(shop: str, tok: str, loc: int, iid: int, delta: int, sku: str):
    if delta == 0:
        return
    if DRY_RUN:
        log(f"[DRY] {shop}: {sku} {delta:+}", lvl="OK")
        return
    shopify(
        shop,
        tok,
        ...
        json={"location_id": loc, "inventory_item_id": iid, "available_adjustment": delta},
    )
...
def offset_sales_from_store2(
    since: datetime,
    vmap1: Dict[str, Variant],
    loc1: int,
    vmap2: Dict[str, Variant],
    tok2: str,
) -> None:
    orders: List[dict] = []
    ...
    qty1 = inventory(SHOP_1, TOKEN_1, loc1, vmap1)  # cache masterkvantiteter
    ...
    for o in orders:
        if o.get("cancelled_at"):
            continue
        order_name = o.get("name") or o.get("id")
        for li in o.get("line_items", []):
            lid = to_int(li["id"])
            if lid == 0 or done(SHOP_2, lid):
                continue
            ...
            connect(SHOP_1, TOKEN_1, iid1, loc1)
            ...
            if after < 0 and not ALLOW_NEG:
                delta = -before
                after = 0
                log(f"⚠️  Negativt lager ej tillåtet – kapar diff för SKU {sku}", lvl="WARN")

            # Detaljerad utskrift för varje rad
            log(
                f"OFFSET {order_name} | line {lid} | SKU {sku} | före={before} sålt={sold} diff={delta:+} efter={after}",
                lvl="INFO",
            )

            adjust(SHOP_1, TOKEN_1, loc1, iid1, delta, sku)
    ...
    # Summering
    log(
        f"SUMMA OFFSET: {total_lines} rad(er), total justering {total_delta:+} artiklar i butik 1",
        lvl="OK",
    )
...
def full_sync_to_store2(
    ...
    for sku, val1 in qty1.items():
        ...
        log(f"SYNC SKU {sku} diff={diff:+}", lvl="INFO")
...
def main():
    ...
    # 2) Synka så att Butik 2 får exakt samma lager som (nu uppdaterade) Butik 1
